...
export class PlayerController extends AvatarController {
    ...
    protected _needUnlockAfterTable:boolean = true;
    ...
    protected catchItem(item:Item, twiceJump:boolean = false) : boolean {
        ...
        switch (item.type) {
            case 0:// money
                ...
                if (this._needUnlockAfterTable && this.sellTable.active) {
                    this._needUnlockAfterTable = false;
                    this.unlockAfterMoney.forEach(node => {
                        if (!node.active) {
                            node.active = true;
                            const orgScale = node.getScale();
                            node.setScale(v3(0.2, 0.2, 0.2));
                            tween(node)
                            .to(0.7, {scale:orgScale}, {easing:'elasticOut'})
                            .start();
                        } else
                            node.active = false;
                    });
